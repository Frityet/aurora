...
static void wait_for_copy_pass_pipelines(const RenderPass& pass) {
  ...
  for (const auto& command : pass.commands) {
    ...
    case ShaderType::Clear:
      pipeline = draw.clear.pipeline;
      break;
    case ShaderType::GX:
      pipeline = draw.gx.pipeline;
      break;
    default:
      continue;
    }
...
static void apply_viewport(const wgpu::RenderPassEncoder& pass, const Viewport& vp) {
  // GX depth scale/offset is represented in the vertex uniform so values
  // outside WebGPU's viewport range remain exact under DepthClipControl.
