...
std::size_t scalar_size(SysConf::Type type) {
  ...
  aurora::throw_host_exception<std::invalid_argument>("SYSCONF has an invalid entry type");
}

std::size_t packed_size(const SysConf::Entry& entry) {
  if (entry.name.empty() || entry.name.size() > 32 || entry.data.empty()) {
    aurora::throw_host_exception<std::invalid_argument>("SYSCONF entry name or payload has an invalid length");
  }
  const auto size = scalar_size(entry.type);
  if (size != 0 && entry.data.size() != size) {
    aurora::throw_host_exception<std::invalid_argument>("SYSCONF integer payload does not match its declared type");
  }
  if ((entry.type == SysConf::Type::SmallArray && entry.data.size() > 256) ||
      (entry.type == SysConf::Type::BigArray && entry.data.size() > 65536)) {
    aurora::throw_host_exception<std::invalid_argument>("SYSCONF array exceeds its wire length field");
  }
...
std::size_t document_size(std::span<const SysConf::Entry> entries) {
  ...
  for (const auto& entry : entries) {
    ...
    if (size > SysConf::FileSize - 4) {
      aurora::throw_host_exception<std::length_error>("SYSCONF entries exceed the 16 KiB document capacity");
    }
  }
  return size;
}
...
std::uint16_t read_u16(std::span<const std::uint8_t> bytes, std::size_t offset) {
  if (offset > bytes.size() || bytes.size() - offset < 2) {
    aurora::throw_host_exception<std::invalid_argument>("SYSCONF truncated halfword");
  }
...
SysConf SysConf::decode(std::span<const std::uint8_t> bytes) {
  ...
  if (bytes.size() < 12 || bytes.size() > FileSize ||
    ...
    aurora::throw_host_exception<std::invalid_argument>("SYSCONF signature or document size is invalid");
  }
  const auto end = bytes.size() - 4;
  const auto count = read_u16(bytes, 4);
  std::size_t position = 6 + 2 * (static_cast<std::size_t>(count) + 1);
  if (position > end) {
    aurora::throw_host_exception<std::invalid_argument>("SYSCONF offset table exceeds the document");
  }
  SysConf result;
  result.m_entries.reserve(count);
  for (std::size_t index = 0; index < count; ++index) {
    if (read_u16(bytes, 6 + 2 * index) != position || position >= end) {
      aurora::throw_host_exception<std::invalid_argument>("SYSCONF entries are not contiguous with their offset table");
    }
    ...
    if (name_length > end - position) {
      aurora::throw_host_exception<std::invalid_argument>("SYSCONF entry name exceeds the document");
    }
    ...
    if (entry.type == Type::BigArray) {
      if (end - position < 2) {
        aurora::throw_host_exception<std::invalid_argument>("SYSCONF missing array length");
      }
    ...
    if (data_length > end - position) {
      aurora::throw_host_exception<std::invalid_argument>("SYSCONF entry payload exceeds the document");
    }
  ...
  if (read_u16(bytes, 6 + 2 * count) != position) {
    aurora::throw_host_exception<std::invalid_argument>("SYSCONF terminal offset does not match its entries");
  }
  return result;
}
...
void SysConf::replace_integer(std::string_view name, Type type, std::uint64_t value) {
  ...
  if (size == 0 || (size < sizeof(value) && value >= (std::uint64_t{1} << (8 * size)))) {
    aurora::throw_host_exception<std::invalid_argument>("SYSCONF value does not fit the requested integer type");
  }
...
void SysConf::erase_at(std::size_t index) {
  if (index >= m_entries.size()) {
    aurora::throw_host_exception<std::out_of_range>("SYSCONF entry index is outside the document");
  }
