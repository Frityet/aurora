...
static void require_array_span(AttrArray& array, u64 offset, u64 length, const char* operation) {
  u32 end = 0;
  AURORA_ASSERT(detail::checked_array_span_end(offset, length, &end),
                "{} array span overflow: offset={}, length={}", operation, offset, length);
  AURORA_ASSERT(end == 0 || array.data != nullptr, "{} read from a null GX array", operation);
  AURORA_ASSERT(!array.sizeKnown || end <= array.size, "{} array read overrun: need {} bytes, have {}", operation,
                end, array.size);
...
ProcessResult process(const u8* data, u32 size) noexcept {
  ...
  while (!reader.empty()) {
    ...
    case CP_CMD_INVAL_VTX: {
      // Invalidate vertex cache and keep draws across the invalidation separate.
...
static void require_draw_array_spans(GXVtxFmt fmt, const u8* data, u16 vtxCount, u32 vtxSize) {
  ...
  for (int i = GX_VA_POS; i <= GX_VA_TEX7; ++i) {
    ...
    require_array_span(array, 0, requiredEnds[i], "indexed draw");
  }
}
...
void handle_aurora(Reader& reader) noexcept {
  ...
  if (subCmd == GX_AURORA_LOAD_VIEWPORT_RENDER) {
    ...
    copy.aa = reader.read<u8>() != 0;
    copy.vfilterEnabled = reader.read<u8>() != 0;
    for (auto& sample : copy.samplePattern) {
      sample[0] = reader.read<u8>();
      sample[1] = reader.read<u8>();
    }
    for (auto& coefficient : copy.vfilter) {
      coefficient = reader.read<u8>();
    }
  } else if (subCmd == GX_AURORA_REQUEST_TAGGED_DEPTH_SNAPSHOT) {
    gfx::request_depth_snapshot(reader.read<u64>());
