// Original RVL ARC/U8 operations. Handles borrow the caller's complete archive
// allocation; no per-open file handle or native heap ownership is introduced.
...
namespace {
int arc_tolower(int c) { return c < 0 || c >= 0x100 ? c : std::tolower(c); }
struct PackedARCHeader {
  aurora::endian::BigEndian<u32> magic, fstStart, fstSize, fileStart, pad[4];
};
} // namespace
...
struct FSTEntry {
  aurora::endian::BigEndian<u32> isDirAndStringOff;
  aurora::endian::BigEndian<u32> parentOrPosition;
  aurora::endian::BigEndian<u32> nextEntryOrLength;
};
...
BOOL ARCInitHandle(void* arcStart, ARCHandle* handle) {
  FSTEntry* FSTEntries;
  const auto* arcHeader = static_cast<const PackedARCHeader*>(arcStart);
  ...
  handle->FSTStart = FSTEntries = reinterpret_cast<FSTEntry*>(static_cast<u8*>(arcStart) + u32(arcHeader->fstStart));
  handle->fileStart = static_cast<u8*>(arcStart) + u32(arcHeader->fileStart);
...
static BOOL isSame(const char* path, const char* string) {
  while (*string != '\0') {
    if (arc_tolower(*path++) != arc_tolower(*string++)) {
      return FALSE;
    }
  }
...
BOOL ARCClose(ARCFileInfo* af) { return TRUE; }
