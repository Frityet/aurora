...
TEST_F(GXFifoTest, DrawIndexed_RoundTripThroughProcessor) {
  std::array<std::array<f32, 3>, 8> positions{};
  std::array<GXColor, 8> colors{};
  std::vector<u8> dl;
  draw_cmd(dl, op(GX_TRIANGLESTRIP, GX_VTXFMT0), 4, {0, 0, 1, 1, 2, 2, 3, 3});
  ...
  gxState().vtxFmts[GX_VTXFMT0].attrs[GX_VA_CLR0] = {GX_CLR_RGBA, GX_RGBA8, 0};
  gxState().arrays[GX_VA_POS] = {positions.data(), sizeof(positions), 0, sizeof(positions[0]), true, true, {}};
  gxState().arrays[GX_VA_CLR0] = {colors.data(), sizeof(colors), 0, sizeof(colors[0]), true, true, {}};
