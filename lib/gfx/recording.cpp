...
DrawCommand make_draw_command(const T& data) {
  ...
  DrawCommand command{.encoder = encode_draw<Renderer, T>, .pipeline = data.pipeline};
...
bool is_frame_active() noexcept {
  // The FIFO worker can advance the active pass while recording copies.
  gx::fifo::drain();
  return g_recorder.currentRenderPass != UINT32_MAX;
}
...
void request_depth_snapshot(uint64_t rawId) noexcept {
  ...
  if (viewportPolicy == AURORA_VIEWPORT_NATIVE) {
    const auto& size = prevPass.colorAttachments[SceneColorAttachmentIndex].size;
    snapshotSize = {size.width, size.height};
  }
